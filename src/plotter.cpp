...
void Plotter::setCurveData(int id, QMap<int, QQueue<QPointF>> *data)
{
#if 0
	for (int index=0; index<id; ++index)
	{
		curveMap[index] = (*data)[index];
	}
#else
	curId = id;
#endif
...
void Plotter::drawCurve(QPainter *painter)
{
	...
	QMapIterator<int, QQueue<QPointF>> i(curveMap);
	while(i.hasNext())
	{
		i.next();
...
void PlotSettings::adjustAxis(double &min, double &max, int &numTicks)
{
	...
	double step = std::pow(10.0,std::log10(grossStep));
